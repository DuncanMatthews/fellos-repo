...
import type { AdminFinderHistoryEntry } from '../../types';

interface HistoryTabProps {
  data: AdminFinderHistoryEntry[];
}
...
const formatChangeValue = (
  ...
  if (
    ...
    const statusReason = value as {
      reason: string | null;
      submitting_user_name: string | null;
    };
    return `${statusReason.reason || 'No reason'} (${statusReason.submitting_user_name || 'Unknown user'})`;
  }
  return String(value);
};
...
export default function HistoryTab({ data }: HistoryTabProps) {
  const renderChangeMessage = (entry: AdminFinderHistoryEntry) => {
    switch (entry.type) {
      case 'status':
        return (
          <>
