...
export function DataTable({ columns, data }: DataTableProps) {
  ...
  React.useEffect(() => {
    try {
      const savedState = sessionStorage.getItem('interactionsTableState');
      if (savedState) {
              ...
              table.getRowModel().rows.map((row) => (
                <TableRow
                  ...
                  className={`cursor-pointer hover:bg-muted/50 ${
                    row.original.is_payment_pending ? 'bg-amber-50' : ''
                  } ${
                    row.original.interaction_status === 'escalated'
                      ? 'bg-red-50'
                      : ''
                  }`}
                >
